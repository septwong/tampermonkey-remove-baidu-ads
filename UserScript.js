...
// @version      1.6
// @description  屏蔽百度/必应搜索网页的引导元素，并去除搜索框自动加载的热搜内容词条，清空 placeholder
// @author       sept
...
(function () {
    ...
    // 必应搜索：隐藏 .sb_form_placeholder 元素
    var bingObserver = new MutationObserver(function () {
        var placeholderEl = document.querySelector('.sb_form_placeholder');
        if (placeholderEl) {
            placeholderEl.style.display = 'none';
            bingObserver.disconnect();
        }
    });

    bingObserver.observe(document.body, {
        childList: true,
        subtree: true
    });
})();
